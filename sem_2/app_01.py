...
def login():
    log_check = {"auth_email": "1@email.ru", "auth_pass": "123"}
    ...
    if request.method == "POST":
        ...
        if (
            auth_email == log_check["auth_email"]
            and auth_pass == log_check["auth_pass"]
        ):
...
def page_not_found():
    context = {"title": "Некорректный возраст", "url": request.base_url}
